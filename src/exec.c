 ...
 */
char exec_rcsid[] =
"$Id: exec.c,v 1.11 2001/11/26 21:51:59 miquels Exp $";
		...
		int exec_wait, char *reply_msg)
{
	...
	if (done) {
		 ...
		 *	for plain text (reply_msg).
		 */
		vp = NULL;
		n = userparse(answer, &vp, 1);
		if (vp) pairfree(vp);
		vp = NULL;
		...
		if (n != 0) {
			...
			if (reply_msg) {
				strncpy(reply_msg, answer, AUTH_STRING_LEN);
				message[AUTH_STRING_LEN - 1] = 0;
			}
		} else {
			/*
			 *	HACK: Replace '\n' with ',' so that
			...
			if (userparse(answer, &vp, 1) != 0)
				log(L_ERR,
		"Exec-Program-Wait: %s: unparsable reply", cmd);
			else {
				replypair_move(reply, &vp, 0);
				pairfree(vp);
			}
		}
	}
